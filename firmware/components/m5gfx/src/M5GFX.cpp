            ...
            if (bus_dsi->init()) {
              ...
              for (int i = 0; !hit_st7121 && !hit_st7123 && !hit_ili9881 && i < 3; ++i) {
                ...
                if (id[0] == 0x71 && id[1] == 0x23) {
                  ...
                  // LOCAL PATCH: the stock code selects the panel from a flaky
                  // I2C touch-FW read and only *logs* this reliable DSI panel
                  // ID. Trust the DSI ID so the panel is detected even when the
                  // touch read races at boot. (Not upstreamed, by request.)
                  hit_st7123 = true;
                  break;
                }
